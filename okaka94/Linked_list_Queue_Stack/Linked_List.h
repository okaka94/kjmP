...
template <class T>										// Box(클래스 템플릿) 객체를 생성할 때 <> 안에 자료형 넣어주는 것 // 여기서는 T를 받아 Box 객체 생성
class Linked_List  
{
	// friend Manage_List;
...
template <class T>
void Linked_List<T>::Init() {          // 생성자에 INIT() 기능 넣기                                                      // 머리,꼬리,중간 초기화
	//S_Head = new Student;
	...
	S_Head->p_next = nullptr;
	S_Next = S_Head;
	counter = 0;                         //student 클래스에서 생성,소멸할때 카운트하니까 일단 주석처리
