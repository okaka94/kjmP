...
void Player_object2D::Set_default() {
	Set_position(40, 40, 15, 15);
}

void Player_object3D::Frame(float delta_time) {
	mass = 100.0f;
	speed = 10.0f;

	Vector Position(box.v_min.x, box.v_min.y, box.v_min.z);
	Vector Key_force = direction * speed * delta_time;	// 랜덤 초기화했던 디렉션 값(벡터)에 스칼라곱한 벡터값 , 델타타임으로 시간동기화

	Add_force(Key_force);									// force 벡터는 기본생성자로 초기값 0.0f -> 키 포스를 더해서 움직이게 함

	acc = force / mass;										// add_force로 더해진 포스 값과 질량을 통해 가속도 구함

	Vector linear_acc = acc * delta_time;					// 델타 타임으로 시간 동기화된 가속도 값 구함
...
void Player_object2D::Frame(float delta_time) {
	mass = 100.0f;
	speed = 10.0f;

	Vector2D Position(rect.x, rect.y);
	Vector2D Key_force = direction2D * speed * delta_time;	// 랜덤 초기화했던 디렉션 값(벡터)에 스칼라곱한 벡터값 , 델타타임으로 시간동기화

	Add_force(Key_force);									// force2D 벡터는 기본생성자로 초기값 0.0f -> 키 포스를 더해서 움직이게 함

	acc2D = force2D / mass;										// add_force로 더해진 포스 값과 질량을 통해 가속도 구함

	Vector2D linear_acc = acc2D * delta_time;					// 델타 타임으로 시간 동기화된 가속도 값 구함
